...
-   **The activity trail is written by a listener, and it is read by more than people.** `RecordActivity` turns every signal into activity rows; the timeline components on a record render them, and the AI assistant queries them through `SearchActivityTool` to answer "who changed this", "what was the price before", "what happened to this customer last week". Anything that changes a record without raising a signal is invisible to both — no timeline entry, and the assistant will truthfully say it does not know.
-   So fill in what that trail needs when you write a signal: `activityDescription()` (the Dutch sentence a person reads), `activityCategory()`, `changes()` with the value before and after per field, `activityContext()` for the other records this fact should also appear on (a work order fact belongs on the customer's timeline too), and `requiredPermission()` when the values are sensitive. An entry whose subject cannot be resolved is never shown, so `subject()` has to be the record the fact is about.
