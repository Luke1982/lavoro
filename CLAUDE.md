...
-   A tenant's own MySQL login gets its rights from the stored procedure `lavoro_admin.grant_tenant_access`, not from a `GRANT` in PHP. MySQL and MariaDB weigh a `GRANT` naming one database against an entry matching that name exactly, never against the wildcard `lavoro_provisioner` holds — so that account can create `lavoro_tenant_acme` and cannot grant on it. The procedure runs as root and refuses anything outside the namespace. `scripts/tenancy/setup-mysql.sh` installs it; `verify-mysql.sh` checks it still refuses.
