...
|---|---|---|
| `demo@lavoro.demo` | `demo` | Sanne de Vries, admin |
| `mark@lavoro.demo` | `demo` | the planner |
| `lisa@lavoro.demo` | `demo` | the service desk |
| `jeroen@lavoro.demo` | `demo` | a mechanic |

Every other demo user logs in the same way: first name `@lavoro.demo`, password
`demo`. The `.demo` domain cannot exist, so these addresses never clash with a
real customer's and no mail can reach them; the tenant has no mail settings
either.
...
- **Real photos instead of the drawings.** The products and people are drawn by
  the seeder. Put a photo in
  `database/seeders/data/demo/photos/products/<brand-model>.jpg` (the slug of
  brand and model, e.g. `daikin-perfera-ftxm25r.jpg`) or
  `database/seeders/data/demo/photos/users/<login>.jpg` (the part of the login
  before the `@`: `mark.jpg`, and `demo.jpg` for Sanne) and the next rebuild
  uses it.
