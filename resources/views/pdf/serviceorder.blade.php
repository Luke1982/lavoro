...
<head>
    ...
    @page { margin: 16mm 18mm; }
        ...
        h1 { text-align: center; text-transform: uppercase; letter-spacing: 4px; font-size: 20px; margin: 0 0 12px; }
        h2 { font-size: 14px; margin: 18px 0 6px; }
        .muted { color: #777; }
        .grid { width: 100%; border-collapse: collapse; }
        .grid td { padding: 4px 6px; vertical-align: top; }
        .grid .label { width: 25%; color: #555; font-size: 11px; }
        .hr { border-top: 1px solid #ddd; margin: 12px 0; }
        .section { margin-top: 8px; }
        .table { width: 100%; border-collapse: collapse; }
        .table th, .table td { padding: 6px; border-bottom: 1px solid #e5e7eb; }
        .table th { text-align: left; font-size: 11px; color: #444; }
    .table.compact th, .table.compact td { padding: 4px 6px; }
        .small { font-size: 11px; }
        .footer { position: fixed; left: 18mm; right: 18mm; bottom: 12mm; font-size: 10px; color: #666; }
    .sign { margin-top: 30mm; margin-bottom: 20mm; }
...
<body>
    ...
    <div class="section small">
        {!! nl2br(e($serviceOrder->description ?? '')) ?: '—' !!}
    </div>
        ...
        <tbody>
            @forelse($serviceOrder->serviceJobs as $job)
                <tr>
                    ...
                    <td>
                        @php
                            $outcomeCase = collect(\App\Enums\ServiceJobOutcomes::cases())->firstWhere('value', $job->outcome);
                        @endphp
                        {{ $outcomeCase?->value ?? '—' }}
                        ...
                        <tr>
                            ...
                            <td>
                                @if($serviceOrder->signature_base64)
                                    <img src="{{ $serviceOrder->signature_base64 }}" alt="Handtekening" style="max-height:80px; max-width:280px; display:block;">
                                @else
                                    —
                                @endif
                            </td>
    ...
    <div class="footer">
        Op al onze offertes, opdrachten en overeenkomsten zijn onze algemene voorwaarden van toepassing. De algemene voorwaarden worden u op verzoek toegezonden.
    </div>
</body>
</html>
