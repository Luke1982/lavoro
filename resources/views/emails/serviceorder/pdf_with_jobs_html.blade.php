...
<body>
    <div class="wrapper">
        ...
        <div class="container">
            ...
            <p>In de bijlage vindt u de PDF van de uitgevoerde werkzaamheden (werkbon) <strong>én</strong> alle
            ...
            @if ($serviceOrder->servicejobs->count())
                ...
                <ul>
                    @foreach ($serviceOrder->servicejobs as $job)
                        <li>Keuring #{{ $job->id }} – {{ $job->asset?->product?->product_type?->name }}
                            {{ $job->asset?->product?->brand?->name }} {{ $job->asset?->product?->model }}
                            ({{ $job->asset?->serial_number }}) – Uitkomst: {{ $job->outcome }}</li>
                    @endforeach
                </ul>
            @endif
