...
<div class="card" style="max-width:100%">
    ...
    <table>
        @foreach($preview['lines'] as $line)
            <tr><td>{{ $line['description'] }}</td><td style="text-align:right;width:140px">@euro($line['amount_cents'])</td></tr>
        @endforeach
        @if($preview['discount_cents'])
            <tr><td>Jaarkorting</td><td style="text-align:right;color:#b91c1c">&minus; @euro($preview['discount_cents'])</td></tr>
        @endif
        <tr><td>Netto</td><td style="text-align:right">@euro($preview['total_cents'])</td></tr>
        <tr><td>BTW {{ $preview['vat_percent'] }}%</td><td style="text-align:right">@euro($preview['vat_cents'])</td></tr>
        <tr><td><strong>Te betalen</strong></td><td style="text-align:right"><strong>@euro($preview['gross_cents'])</strong></td></tr>
    </table>
    @if($is_due)
    ...
    @forelse($invoices as $invoice)
        <table style="margin-bottom:14px">
            <tr>
                ...
                <th style="text-align:right">@euro($invoice->gross_cents)</th>
            ...
            @foreach($invoice->lines as $line)
                <tr><td colspan="2" class="muted">{{ $line->description }}</td>
                    <td style="text-align:right" class="muted">@euro($line->amount_cents)</td></tr>
            @endforeach
