<template>
    <div data-planner-event
        ...
        <VDropdown :triggers="popoverTriggers" :disabled="!isShort || isBeingDragged" :delay="{ show: 200, hide: 80 }"
            ...
            <div class="px-3 py-2 flex flex-col h-full justify-between overflow-hidden"
                ...
                <button v-if="!isCompact && event.eventable_id"
                    ...
                    <span class="truncate">WB-{{ String(event.eventable_id).padStart(4, '0') }}<template
                            v-if="event.project_name"> van {{ event.project_name }}</template></span>
        ...
        <div v-if="event.from_google" class="absolute top-1 right-2 pointer-events-none"
            v-tooltip="'Afkomstig uit Google Calendar'">
