<template>
    <div>
        <div class="p-6 border border-gray-300 dark:border-slate-700/60 rounded-md bg-gray-50 dark:bg-slate-900 relative pt-8 mt-5">
            <div class="absolute left-4 -top-5 bg-white dark:bg-slate-900 p-3 border border-gray-400 dark:border-slate-600 rounded-full flex">
            ...
            <ul role="list" class="space-y-6 max-h-[20rem] overflow-y-scroll" v-auto-animate>
                <li v-for="(comment, idx) in comments" :key="comment.id" class="relative flex gap-x-4 bg-gray-100 dark:bg-slate-800/40">
                    <div
                    ...
                    <div class="flex-auto rounded-md p-3 ring-1 ring-inset ring-gray-200 dark:ring-slate-700/60 pr-10 bg-white dark:bg-slate-800">
                        <div class="flex justify-between gap-x-4">
                            <div class="py-0.5 text-xs/5 text-gray-500 dark:text-slate-400">
                                <span class="font-medium text-gray-900 dark:text-slate-200">{{ comment.user.name }}</span> maakte een
                                opmerking
                            </div>
                            <time :datetime="comment.created_at" class="flex-none py-0.5 text-xs/5 text-gray-500 dark:text-slate-500">
                    ...
                    <TrashIcon v-if="page.props.auth.user.id === comment.user_id"
                        class="h-5 w-5 text-red-400 dark:text-red-400 cursor-pointer absolute top-3 right-3 z-[50]" aria-hidden="true"
            ...
            <div class="mt-6 flex gap-x-3">
                <div class="relative flex-auto">
                    ...
                    <div class="absolute inset-x-0 bottom-0 flex justify-end py-2 pl-3 pr-2 bg-white dark:bg-slate-800/95">
