<template>
    <Link :href="`/tickets/${ticket.id}`" :class="[getWrapperClasses(ticket.status), 'rounded-md p-2 border-2 block']">
    <div class="flex">
        <div class="flex flex-col items-center justify-start mr-2 w-20">
            <ExclamationCircleIcon :class="[getExclamationMarkClasses(ticket.priority, ticket.status), 'w-6 h-6']" />
...
<script setup>
import { CalendarDateRangeIcon, ExclamationCircleIcon } from '@heroicons/vue/24/outline';
